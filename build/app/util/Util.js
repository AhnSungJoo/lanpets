...
class Util {
    ...
    static areDefinedIn(obj, properties) {
    ...
    static areDefinedIn2(obj, properties) {
        const keys = Object.keys(obj);
        for (let i = 0; i < properties.length; ++i) {
            const prop = properties[i];
            const result = keys.filter((key) => key === prop);
            if (result.length === 0)
                return false;
        }
        return true;
    ...
    static sliceAndPush(src, dst, finalSize) {
    ...
    static removeElementAt(arr, idx) {
        const pre = arr.splice(0, idx);
        const post = arr.splice(1);
    ...
    static getRandomID(splitSectionLen = 5) {
