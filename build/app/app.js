...
logger_1.default.info(`app name : ${config.get('name')}`);
...
logger_1.default.info(`ENV : ${ENV}`);
logger_1.default.info(`process env : ${process.env.NODE_ENV}, ${config.get('host')}`);
logger_1.default.info(`db info :${JSON.stringify(process.env.db)}`);
koaApp.on('error', (err) => {
    if (ENV === 'dev') {
